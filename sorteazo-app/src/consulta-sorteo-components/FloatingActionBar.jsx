...
export const FloatingActionBar = ({ count, totalPrice, onActionClick, mode = 'apartar' }) => {
    const buttonText = mode === 'pagar' ? 'Pagar Boletos' : 'Apartar Boletos';
    const ButtonIcon = mode === 'pagar' ? FaCreditCard : FaShoppingCart;
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white shadow-[0_-2px_10px_rgba(0,0,0,0.1)] z-40">
            <div className="container mx-auto px-8 py-4 max-w-5xl flex justify-between items-center">
                <div>
                    ...
                    <p className="text-green-600 font-semibold">Total: ${totalPrice.toFixed(2)}</p>
                </div>
                <button
